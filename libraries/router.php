		...
		public static function dispatch($format) {
			...
			if (class_exists($class_name)) {
				...
				if (is_callable(array($tmp_class, $action)))
					...
					die('The action <strong>' . $action . '</strong> could not be called from the controller <strong>'.$class_name.'</strong> found in <pre>' . APP_PATH . 'controllers/' . $controller . '_controller.php</pre>');
			} else {
				die('The class <strong>' . $class_name . '</strong> could no be found in <pre>' . APP_PATH . 'controllers/' . $controller . '_controller.php</pre>');
			}
			...
			if (!empty($layout_path)) {
				...
				// replace {PAGE_CONTENT} with the view file
